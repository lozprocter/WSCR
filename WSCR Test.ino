...
//SETS ALL RESULT LEDS GREEN
  ...
  attachInterrupt(digitalPinToInterrupt(cable_1), cable_1_fail, LOW);

  pinMode(cable_2, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(cable_2), cable_2_fail, LOW);

  pinMode(cable_3, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(cable_3), cable_3_fail, LOW);

  pinMode(cable_4, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(cable_4), cable_4_fail, LOW);

  pinMode(cable_5, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(cable_5), cable_5_fail, LOW);
...
void setup() {
  ...
  if (!oled.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
      ...
      oled.println("PRESS YELLOW BUTTON"); // text to display
